...
export default function TeacherLogin() {
  ...
  const submit = async () => {
    ...
    setError('')
    try {
      const res = await loginTeacher({ username: username.trim(), password })
      setTeacherRoomSession({ token: res.token, user: res.user })
      showToast('讲师登录成功')
    ...
    <div className="min-h-screen bg-canvas px-6 py-10 text-ink dark:bg-slate-950 dark:text-slate-100">
      <main className="mx-auto max-w-md">
        <Card padded className="!p-6">
          <p className="text-sm font-semibold text-brand-600 dark:text-brand-100">讲师端</p>
          <h1 className="mt-2 text-2xl font-semibold tracking-normal">账号登录</h1>
          {error && (
